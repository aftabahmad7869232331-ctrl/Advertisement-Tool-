...
const AuthService = {
  ...
  initiateOAuth(provider: OAuthProvider): void {
    if (provider === 'email') {
      throw new Error('Email authentication does not use the OAuth redirect flow');
    }

    type SocialOAuthProvider = Exclude<OAuthProvider, 'email'>;
    const socialProvider: SocialOAuthProvider = provider;
    ...
    const urls: Record<SocialOAuthProvider, string> = {
    ...
    const providerUrl = urls[socialProvider];

    if (!providerUrl) {
      throw new Error(`OAuth URL is not configured for "${provider}"`);
    }
  ...
  async enable2FA(): AsyncResult<TwoFactorSetup> {
    ...
    try {
      ...
      return this._networkError<TwoFactorSetup>(err);
    }
  },
  ...
  async verify2FACode(code: string): AsyncResult<{ verified: boolean }> {
    try {
      ...
      return this._networkError<{ verified: boolean }>(err);
    }
  },
  ...
  async disable2FA(code: string): AsyncResult<{ disabled: boolean }> {
    try {
      ...
      return this._networkError<{ disabled: boolean }>(err);
    }
  },
  ...
  async createApiKey(name: string, scopes: string[] = []): AsyncResult<ApiKey & { rawKey: string }> {
    try {
      ...
      return this._networkError<ApiKey & { rawKey: string }>(err);
    }
  },
  ...
  async getApiKeys(): AsyncResult<ApiKey[]> {
    try {
      ...
      return this._networkError<ApiKey[]>(err);
    }
  },
  ...
  async revokeApiKey(keyId: string): AsyncResult<{ revoked: boolean }> {
    try {
      ...
      return this._networkError<{ revoked: boolean }>(err);
    }
  },
  ...
  async fetchCurrentUser(): AsyncResult<User> {
    try {
      ...
      return this._networkError<User>(err);
    }
  },
  ...
  _networkError<T>(err: unknown): ApiResponse<T> {
